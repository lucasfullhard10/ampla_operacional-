...
const TAB_TITLES: Record<string, string> = {
  ...
  motoristas: "Pessoas (Cadastro Unificado)",
  "central-documentos": "Central de Documentos & Conformidade",
...
export default function App() {
  ...
  const itemsSidemenu = [
    ...
    { id: "motoristas", label: "Cadastro de Pessoas", icon: Users },
    { id: "central-documentos", label: "Central de Documentos", icon: FolderCheck },
    ...
    { id: "devolucoes", label: "Gestão Devoluções", icon: Undo2 },
    ...
    // Role based security rules for "Contas a Receber"
    if (m.id === "recebimentos") {
      const type = currentUser.tipo_usuario || "";
      const profile = currentUser.perfil || "";
      if (type === "OPERADOR" || profile === "operador" || type === "CONFERENTE" || type === "MOTORISTA") {
        return false;
      }
    }
      ...
      <div className={`min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col md:flex-row transition-all duration-300 ${compactMode ? "ultra-compact text-xs" : ""}`}>
        ...
        <main className="flex-1 flex flex-col min-w-0">
          ...
          <section className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-850">
            ...
            {activeTab === "central-documentos" && (
              <CentralDocumentosView 
                motoristas={motoristas}
                unidades={unidades}
                onRefresh={loadGlobalData}
                userEmail={currentUser.email}
            ...
            {activeTab === "financeiro-pessoas" && (
              <FinanceiroPessoasView 
                currentUser={currentUser!}
                unidades={unidades}
                onRefresh={loadGlobalData}
              />
