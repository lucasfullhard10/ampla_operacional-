...
import { 
  Calendar, Save, FileSpreadsheet, RefreshCcw, Search, UserCheck, User,
...
export default function DisponibilidadeView({ veiculos, motoristas, userEmail }: DisponibilidadeProps) {
  ...
  const [activeTab, setActiveTab] = useState<"centralPatio" | "cadastro" | "registros" | "ociosidade" | "relatorios" | "graficos" | "historico">("centralPatio");
  const [savingMotId, setSavingMotId] = useState<string | null>(null);

  // Yard Control filters & records
  const [yardSearch, setYardSearch] = useState("");
  const [yardFilterType, setYardFilterType] = useState<"Todos" | "Motoristas" | "Ajudantes" | "Veículos">("Todos");
  const [yardFilterHelperType, setYardFilterHelperType] = useState("Todos");
  const [yardFilterVehiclePerfil, setYardFilterVehiclePerfil] = useState("Todos");
  const [yardFilterStatus, setYardFilterStatus] = useState("Todos");
  ...
  const [helpersAvailable, setHelpersAvailable] = useState<Record<string, boolean>>({});
  ...
  // Central Operational Status Calculation (Fase 9)
  const getPersonStatus = (m: Motorista) => {
    ...
    const activeDt = rotas.find(r => 
      r.data === selectedDate && 
      (r.motoristaId === m.id || (r.ajudantesIds && r.ajudantesIds.includes(m.id)))
    );
    if (activeDt) {
      const sv = activeDt.status_viagem?.toLowerCase() || activeDt.status?.toLowerCase() || "";
      if (sv.includes("rota") || sv.includes("descarga") || sv.includes("entregando")) {
        return { status: "Em Rota", icon: "🟣", color: "text-purple-400", bgColor: "bg-purple-500/10", borderColor: "border-purple-500/20" };
      } else if (sv.includes("carregamento") || sv.includes("carga")) {
        return { status: "Escalado", icon: "🔵", color: "text-blue-400", bgColor: "bg-blue-500/10", borderColor: "border-blue-500/20" };
      }
    }
    const isDeclared = rawDisps.some(d => d.data === selectedDate && (d.motoristaId === m.id || d.veiculoId === `helper-${m.id}`));
    if (isDeclared) {
      return { status: "Disponível", icon: "🟢", color: "text-emerald-400", bgColor: "bg-emerald-500/10", borderColor: "border-emerald-500/20" };
    }
    return { status: "Ausente", icon: "⚫", color: "text-slate-450", bgColor: "bg-slate-500/10", borderColor: "border-slate-500/20" };
  };

  const isVeiculoBlocked = (v: Veiculo) => {
    ...
    return v.status === "Bloqueado" || isLicExpired || isSegExpired || isMaintCriticalExpired;
  };

  const getVehicleStatus = (v: Veiculo) => {
    if (isVeiculoBlocked(v)) {
      return { status: "Bloqueado", icon: "🔴", color: "text-rose-500", bgColor: "bg-rose-500/10", borderColor: "border-rose-500/20" };
    }
    const activeDt = rotas.find(r => r.data === selectedDate && r.veiculoId === v.id);
    if (activeDt) {
      const sv = activeDt.status_viagem?.toLowerCase() || activeDt.status?.toLowerCase() || "";
      if (sv.includes("rota") || sv.includes("descarga") || sv.includes("entregando")) {
        return { status: "Em Rota", icon: "🟣", color: "text-purple-400", bgColor: "bg-purple-500/10", borderColor: "border-purple-500/20" };
    ...
    today.setHours(0, 0, 0, 0);
    const activeMaint = manutencoes.find(m => 
      m.veiculoId === v.id && 
      new Date(m.data) <= today && 
      new Date(m.proximaManutencao) >= today
    );
    if (activeMaint) {
      return { status: "Manutenção", icon: "🟠", color: "text-amber-500", bgColor: "bg-amber-500/10", borderColor: "border-amber-500/20" };
    }
    const isDeclared = rawDisps.some(d => d.data === selectedDate && d.veiculoId === v.id);
    if (isDeclared) {
      return { status: "Disponível", icon: "🟢", color: "text-emerald-400", bgColor: "bg-emerald-500/10", borderColor: "border-emerald-500/20" };
    }
    return { status: "Disponível", icon: "🟢", color: "text-emerald-400", bgColor: "bg-emerald-500/10", borderColor: "border-emerald-500/20" };
  ...
  const getFilteredYardResources = () => {
    const list: any[] = [];
    const search = yardSearch.toLowerCase().trim();

    if (yardFilterType === "Todos" || yardFilterType === "Motoristas" || yardFilterType === "Ajudantes") {
      motoristas.forEach(m => {
        const isDriver = m.tipo === "Motorista" || !m.tipo;
        if (yardFilterType === "Motoristas" && !isDriver) return;
        if (yardFilterType === "Ajudantes" && isDriver) return;
        
        if (yardFilterHelperType !== "Todos" && m.tipo !== yardFilterHelperType) return;

        const opStatus = getPersonStatus(m);
        if (yardFilterStatus !== "Todos" && opStatus.status !== yardFilterStatus) return;
        if (selectedUnit !== "Todas" && m.unidadeId !== selectedUnit) return;

        const matchesSearch = 
          m.nome.toLowerCase().includes(search) || 
          m.cpf.toLowerCase().includes(search) ||
          (m.unidadeId || "").toLowerCase().includes(search);

        if (matchesSearch) {
          list.push({
            id: m.id,
            resType: "Pessoa",
            role: m.tipo || "Motorista",
            name: m.nome,
            identifier: m.cpf,
            unidadeId: m.unidadeId,
            opStatus,
            alert: getPreventiveAlertsForResource(m.id, "Pessoa"),
            raw: m
          });
        }
      });
    ...
    if (yardFilterType === "Todos" || yardFilterType === "Veículos") {
      veiculos.forEach(v => {
        if (yardFilterVehiclePerfil !== "Todos" && v.perfil !== yardFilterVehiclePerfil) return;

        const opStatus = getVehicleStatus(v);
        if (yardFilterStatus !== "Todos" && opStatus.status !== yardFilterStatus) return;
        if (selectedUnit !== "Todas" && v.unidadeId !== selectedUnit) return;

        const matchesSearch = 
          v.placa.toLowerCase().includes(search) || 
          v.modelo.toLowerCase().includes(search) ||
          v.marca.toLowerCase().includes(search) ||
          (v.unidadeId || "").toLowerCase().includes(search);
        ...
        if (matchesSearch) {
          list.push({
            ...
            role: v.perfil || "Truck",
            name: v.placa,
            identifier: `${v.marca} ${v.modelo} (${v.ano})`,
            unidadeId: v.unidadeId,
            opStatus,
            alert: getPreventiveAlertsForResource(v.id, "Veículo"),
            raw: v
          });
        }
      });
  ...
  // Dynamic KPI Counts calculations for Central do Patio
  const calculatedDrivers = motoristas.filter(m => m.tipo === "Motorista" || !m.tipo);
  const calculatedHelpers = motoristas.filter(m => m.tipo === "Ajudante Fixo" || m.tipo === "Ajudante Geral");

  const driversWithStatus = calculatedDrivers.map(m => ({ ...m, opStatus: getPersonStatus(m) }));
  const helpersWithStatus = calculatedHelpers.map(m => ({ ...m, opStatus: getPersonStatus(m) }));
  const vehiclesWithStatus = veiculos.map(v => ({ ...v, opStatus: getVehicleStatus(v) }));

  const countDriversDisp = driversWithStatus.filter(d => d.opStatus.status === "Disponível").length;
  const countDriversRota = driversWithStatus.filter(d => d.opStatus.status === "Em Rota" || d.opStatus.status === "Escalado").length;
  const countDriversBloq = driversWithStatus.filter(d => d.opStatus.status === "Bloqueado").length;
  const countHelpersDisp = helpersWithStatus.filter(h => h.opStatus.status === "Disponível").length;

  const countVehiclesDisp = vehiclesWithStatus.filter(v => v.opStatus.status === "Disponível").length;
  const countVehiclesRota = vehiclesWithStatus.filter(v => v.opStatus.status === "Em Rota" || v.opStatus.status === "Escalado").length;
  const countVehiclesBloq = vehiclesWithStatus.filter(v => v.opStatus.status === "Bloqueado").length;
  const countVehiclesMaint = vehiclesWithStatus.filter(v => v.opStatus.status === "Manutenção").length;
  ...
  const fetchAvailabilities = async () => {
    const isRegistersOrOtherTabs = activeTab !== "cadastro" && activeTab !== "centralPatio";
    setLoading(true);
    try {
      try {
        ...
        const resMaint = await fetch("/api/manutencao", {
          headers: { "x-user-email": userEmail }
        });
        if (resMaint.ok) {
          const dataMaint = await resMaint.json();
          setManutencoes(dataMaint);
        }
      } catch (err) {
        console.error("Erro ao carregar dados operacionais secundários:", err);
      }
      ...
      if (res.ok) {
        ...
        if (isRegistersOrOtherTabs || periodFilter === "Dia" || activeTab === "centralPatio") {
          ...
          data.forEach(d => {
            if (d.data === selectedDate && d.veiculoId && d.veiculoId.startsWith("helper-")) {
              const helperId = d.veiculoId.replace("helper-", "");
              hState[helperId] = true;
            }
          });
          setHelpersAvailable(hState);
        }
      }
    } catch (e) {
  ...
  const handleToggleHelper = (hId: string) => {
    setHelpersAvailable((prev) => ({
      ...prev,
      [hId]: !prev[hId],
    }));
    setHasUnsavedChanges(true);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    let hasBlockedDriver = false;
    let blockedDriverName = "";
    let blockedDriverReason = "";

    Object.entries(formState).forEach(([veiculoId, val]) => {
      const v = val as any;
      if (v.isAvailable && v.driverId) {
        const mot = motoristas.find(m => m.id === v.driverId);
        if (mot && mot.statusFinal === "BLOQUEADO") {
          hasBlockedDriver = true;
          blockedDriverName = mot.nome;
          blockedDriverReason = mot.motivoBloqueio || "Inconformidade cadastral";
        }
      }
    });
    ...
    if (hasBlockedDriver) {
      setNotification({
        type: "error",
        message: `SALVAMENTO BLOQUEADO\n\n❌ O motorista ${blockedDriverName} está BLOQUEADO: ${blockedDriverReason}.`
      });
      setSaving(false);
      return;
    }
    ...
    // Save helper availability
    Object.entries(helpersAvailable).forEach(([helperId, isAvailable]) => {
      if (isAvailable) {
        payloadList.push({
          data: selectedDate,
          veiculoId: `helper-${helperId}`,
          unidadeId: targetUnit,
          prioridade: "Média",
          motoristaId: helperId,
          roteirizado: false,
          motivoOciosidade: ""
        });
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-slate-800 flex flex-wrap gap-2 pt-2">
        <button
          onClick={() => setActiveTab("centralPatio")}
          className={`py-2 px-4 text-xs font-semibold rounded-t-lg transition font-mono ${
            activeTab === "centralPatio"
              ? "bg-slate-900 border-t-2 border-emerald-500 text-emerald-400 font-bold"
              : "text-slate-400 hover:text-white"
          }`}
        >
          ⚡ Central do Pátio (Fase 9)
        </button>
        <button
      ...
      {activeTab === "centralPatio" && (
        <div className="space-y-6">
          {/* PAINEL DE INDICAÇÃO SUPERIOR (METRICS KPI) */}
          <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-4">
            ...
            <div className="bg-slate-900/90 p-4 rounded-xl border border-slate-800/80 shadow-md">
              <div className="flex items-center justify-between border-b border-slate-800/60 pb-2 mb-3">
                <span className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
                  👤 Condutores
                </span>
                <span className="text-[10px] bg-sky-500/10 text-sky-400 px-2 py-0.5 rounded-full font-mono border border-sky-500/20">
                  Total: {calculatedDrivers.length}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-emerald-400 font-bold font-mono">Disponíveis</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countDriversDisp}</span>
                </div>
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-purple-400 font-bold font-mono">Em Rota</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countDriversRota}</span>
                </div>
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-rose-500 font-bold font-mono">Bloqueados</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countDriversBloq}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 p-4 rounded-xl border border-slate-800/80 shadow-md">
              <div className="flex items-center justify-between border-b border-slate-800/60 pb-2 mb-3">
                ...
                <span className="text-[10px] bg-indigo-500/10 text-indigo-400 px-2 py-0.5 rounded-full font-mono border border-indigo-500/20">
                  Total: {calculatedHelpers.length}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-center">
                <div className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-emerald-400 font-bold font-mono">Disponíveis</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countHelpersDisp}</span>
                </div>
                <div className="bg-slate-950/40 p-2.5 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-slate-400 font-bold font-mono">Em Rota / Ausentes</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{calculatedHelpers.length - countHelpersDisp}</span>
                </div>
              </div>
            </div>

            {/* KPI Veículos */}
            <div className="bg-slate-900/90 p-4 rounded-xl border border-slate-800/80 shadow-md xl:col-span-2">
              <div className="flex items-center justify-between border-b border-slate-800/60 pb-2 mb-3">
                <span className="text-xs font-bold font-mono text-slate-400 uppercase tracking-wide flex items-center gap-1.5">
                  🚛 Frotas / Veículos
                </span>
              ...
              <div className="grid grid-cols-4 gap-2 text-center">
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-emerald-400 font-bold font-mono">Disponíveis</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countVehiclesDisp}</span>
                </div>
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-purple-400 font-bold font-mono">Em Rota</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countVehiclesRota}</span>
                </div>
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-rose-500 font-bold font-mono">Bloqueados</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countVehiclesBloq}</span>
                </div>
                <div className="bg-slate-950/40 p-2 rounded-lg border border-slate-850">
                  <span className="block text-[10px] text-amber-500 font-bold font-mono">Manutenção</span>
                  <span className="text-xl font-extrabold text-white block mt-1">{countVehiclesMaint}</span>
                </div>
              </div>
            </div>
          ...
          {/* GRID PRINCIPAL: SIDEBAR FILTROS & PAINEL DE CONTROLE */}
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
            
            {/* COLUNA ESQUERDA (SIDEBAR DE CONTROLE DE DATA, ALERTAS E AÇÕES) */}
            <div className="space-y-6 lg:col-span-1">
              
              {/* Card de Data & Unidade */}
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-4 shadow-sm">
                <h3 className="text-xs font-bold font-mono uppercase text-slate-400 tracking-wider flex items-center gap-1.5 pb-2 border-b border-slate-800">
                  ⚙️ Parâmetros Operacionais
                </h3>
                
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Unidade do Pátio</label>
                  <select
                    value={selectedUnit}
                    onChange={(e) => setSelectedUnit(e.target.value)}
                    className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2 text-xs font-semibold font-mono"
                  >
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-mono text-slate-400 uppercase tracking-wider block">Data de Controle</label>
                  <div className="relative">
                    <input
                      type="date"
                      value={selectedDate}
                      onChange={(e) => setSelectedDate(e.target.value)}
                      className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2 text-xs font-semibold font-mono pl-8"
                    />
                ...
                <button 
                  onClick={fetchAvailabilities}
                  className="w-full py-2 bg-slate-800 hover:bg-slate-750 text-slate-200 rounded-lg text-xs font-mono font-bold flex items-center justify-center gap-1.5 transition border border-slate-700/50"
                >
                  <RefreshCcw className="w-3.5 h-3.5" /> Atualizar Pátio
                </button>
              </div>

              {/* Banner de Alterações Pendentes */}
              {hasUnsavedChanges && (
                <div className="bg-amber-500/15 border-2 border-amber-500/30 p-4 rounded-xl space-y-3 shadow-md animate-pulse">
                  <div className="flex items-start gap-2">
                    <AlertCircle className="w-5 h-5 text-amber-400 shrink-0 mt-0.5" />
                    <div>
                      <h4 className="text-xs font-bold text-amber-300 font-mono">Alterações Pendentes</h4>
                      <p className="text-[10px] text-amber-400/90 leading-normal mt-1">
                        Você alterou a disponibilidade de recursos. Estas mudanças ainda não foram gravadas no banco de dados.
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={handleSave}
                    disabled={saving}
                    className="w-full py-2.5 bg-amber-500 hover:bg-amber-400 disabled:bg-amber-600 text-slate-950 font-extrabold rounded-lg text-xs font-mono tracking-wider flex items-center justify-center gap-1.5 shadow-lg transition duration-200"
                  >
                    <Save className="w-4 h-4" /> {saving ? "Salvando..." : "GRAVAR NO BANCO"}
                  </button>
                </div>
              )}

              {/* Card de Alertas Preventivos */}
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-3 shadow-sm">
                <h3 className="text-xs font-bold font-mono uppercase text-amber-500 tracking-wider flex items-center gap-1.5 pb-2 border-b border-slate-800">
                  ⚠️ Alertas Preventivos
                </h3>
                {getPreventiveAlerts().length === 0 ? (
                  <div className="text-center py-4 text-xs text-slate-500 font-mono">
                    Nenhum vencimento iminente detectado no pátio.
                  </div>
                ) : (
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {getPreventiveAlerts().slice(0, 5).map((alert, idx) => (
                      <div 
                        key={alert.id}
                        className={`p-2 rounded-lg border text-[10px] font-mono flex items-start gap-1.5 ${
                      ...
                      >
                        ...
                        <div>
                          <span className="font-extrabold block text-[9px] uppercase tracking-wider">{alert.name} ({alert.type})</span>
                          <span className="block mt-0.5 leading-snug">{alert.msg}</span>
                        </div>
                      </div>
                    ))}
                    {getPreventiveAlerts().length > 5 && (
                      <div className="text-center text-[9px] text-slate-500 font-mono pt-1">
                        + {getPreventiveAlerts().length - 5} alertas cadastrais ocultos
                      </div>
                    )}
                  </div>
                )}
              ...
              {/* Card de Legenda */}
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 text-[10px] font-mono space-y-2 shadow-sm text-slate-400">
                <h4 className="font-bold text-xs uppercase text-slate-300 border-b border-slate-800 pb-1.5 mb-2">Ciclo Operacional</h4>
                <div className="grid grid-cols-2 gap-2">
                  <div className="flex items-center gap-1"><span className="text-xs">🟢</span> <span>Disponível</span></div>
                  <div className="flex items-center gap-1"><span className="text-xs">🔵</span> <span>Escalado (Carga)</span></div>
                  <div className="flex items-center gap-1"><span className="text-xs">🟣</span> <span>Em Rota (DT)</span></div>
                  <div className="flex items-center gap-1"><span className="text-xs">🔴</span> <span>Bloqueado</span></div>
                  <div className="flex items-center gap-1"><span className="text-xs">⚫</span> <span>Ausente</span></div>
                  <div className="flex items-center gap-1"><span className="text-xs">🟠</span> <span>Manutenção</span></div>
                </div>
              </div>

            </div>

            {/* COLUNA DIREITA (GRID INTELIGENTE DE RECURSOS) */}
            <div className="lg:col-span-3 space-y-4">
              ...
              <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 space-y-3.5">
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="Buscar recurso por Nome, CPF, Placa, Modelo ou Unidade..."
                    className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2.5 text-xs font-semibold pl-9 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition outline-none"
                  />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-mono text-slate-400 uppercase">Filtro de Recurso</label>
                    <select
                      value={yardFilterType}
                      onChange={(e: any) => {
                        setYardFilterType(e.target.value);
                        setYardFilterHelperType("Todos");
                        setYardFilterVehiclePerfil("Todos");
                      }}
                      className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2 text-xs font-mono font-semibold"
                    >
                      <option value="Todos">👥/🚛 Todos Recursos</option>
                      <option value="Motoristas">👤 Apenas Condutores</option>
                      <option value="Ajudantes">🤝 Apenas Ajudantes</option>
                      <option value="Veículos">🚛 Apenas Veículos</option>
                    </select>
                  </div>

                  {/* Sub-filtros dinâmicos baseados no tipo */}
                  {yardFilterType === "Ajudantes" ? (
                    <div className="space-y-1">
                      <label className="text-[10px] font-mono text-slate-400 uppercase">Categoria de Ajudante</label>
                      <select
                        value={yardFilterHelperType}
                        onChange={(e) => setYardFilterHelperType(e.target.value)}
                        className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2 text-xs font-mono font-semibold"
                      >
                        <option value="Todos">Todos Ajudantes</option>
                        <option value="Ajudante Fixo">Ajudante Fixo</option>
                        <option value="Ajudante Geral">Ajudante Geral</option>
                      </select>
                    </div>
                  ) : yardFilterType === "Veículos" ? (
                    <div className="space-y-1">
                      <label className="text-[10px] font-mono text-slate-400 uppercase">Perfil do Veículo</label>
                      <select
                        value={yardFilterVehiclePerfil}
                        onChange={(e) => setYardFilterVehiclePerfil(e.target.value)}
                        className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2 text-xs font-mono font-semibold"
                      >
                    ...
                    <div className="space-y-1 opacity-50">
                      <label className="text-[10px] font-mono text-slate-400 uppercase">Filtro Secundário</label>
                      <select
                        disabled
                        className="w-full bg-slate-950 text-white rounded-lg border border-slate-850 p-2 text-xs font-mono font-semibold"
                      >
                  ...
                  {/* Status Operacional */}
                  <div className="space-y-1">
                    <label className="text-[10px] font-mono text-slate-400 uppercase">Status Operacional</label>
                    <select
                      value={yardFilterStatus}
                      onChange={(e) => setYardFilterStatus(e.target.value)}
                      className="w-full bg-slate-950 text-white rounded-lg border border-slate-800 p-2 text-xs font-mono font-semibold"
                    >
                  ...
                  <div className="flex items-end">
                    <button
                      onClick={() => {
                        setYardSearch("");
                        setYardFilterType("Todos");
                        setYardFilterHelperType("Todos");
                        setYardFilterVehiclePerfil("Todos");
                        setYardFilterStatus("Todos");
                      }}
                      className="w-full py-2 bg-slate-950 border border-slate-800 hover:bg-slate-850 text-slate-400 hover:text-white rounded-lg text-xs font-mono transition"
                    >
                      🧹 Limpar Filtros
                    </button>
                  </div>
              ...
              {/* Grid dos Recursos Filtrados */}
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                
                {getFilteredYardResources().length === 0 ? (
                  <div className="col-span-full bg-slate-900 border border-slate-800 p-8 rounded-xl text-center space-y-2">
                    <div className="text-4xl">🔍</div>
                    <h3 className="text-sm font-bold text-slate-300 font-mono">Nenhum recurso encontrado</h3>
                    <p className="text-xs text-slate-500 max-w-sm mx-auto">
                      Não encontramos nenhum condutor, ajudante ou veículo correspondente aos termos de busca e filtros ativos.
                    </p>
                  </div>
                ) : (
                  getFilteredYardResources().map((res) => {
                    const op = res.opStatus;
                    const isPerson = res.resType === "Pessoa";
                    const isDriver = isPerson && (res.role === "Motorista" || !res.role);
                    const isHelper = isPerson && !isDriver;

                    // Figure out checkbox checking state
                    let isChecked = false;
                    let isBlocked = op.status === "Bloqueado" || op.status === "Manutenção" || op.status === "Em Rota" || op.status === "Escalado";

                    if (isPerson) {
                      if (isDriver) {
                        // Find vehicle has this driver
                        const matchVeh = veiculos.find(v => formState[v.id]?.driverId === res.id);
                        isChecked = !!(matchVeh && formState[matchVeh.id]?.isAvailable);
                      } else {
                        isChecked = !!helpersAvailable[res.id];
                      }
                    } else {
                      isChecked = !!formState[res.id]?.isAvailable;
                    }

                    return (
                      <div 
                        key={`${res.resType}-${res.id}`}
                        className={`bg-slate-900 border rounded-xl overflow-hidden shadow-sm flex flex-col justify-between transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md ${
                          isChecked 
                      ...
                      >
                        ...
                        <div className="p-4 space-y-3">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2.5">
                              ...
                              <div className={`w-10 h-10 rounded-lg flex items-center justify-center border shrink-0 ${
                                isChecked ? "bg-emerald-500/10 border-emerald-500/20" : "bg-slate-950 border-slate-800"
                              }`}>
                                {isPerson ? (
                                  <User className={`w-5 h-5 ${isChecked ? "text-emerald-400" : "text-slate-400"}`} />
                                ) : (
                                  <Truck className={`w-5 h-5 ${isChecked ? "text-emerald-400" : "text-slate-400"}`} />
                                )}
                              </div>
                              ...
                              <div className="min-w-0">
                                <span className={`text-[10px] font-mono font-bold uppercase tracking-wider block ${
                                  ...
                                  {isPerson ? `👤 ${res.role}` : `🚛 ${res.role}`}
                                </span>
                                <h4 className="text-xs font-extrabold text-white font-mono truncate leading-tight mt-0.5" title={res.name}>
                                  {res.name}
                                </h4>
                                <span className="text-[10px] text-slate-500 font-mono block mt-0.5">{res.identifier}</span>
                              </div>
                            </div>

                            {/* Status Operational Badge */}
                            <div className={`px-2 py-1 rounded-lg border text-[10px] font-extrabold font-mono flex items-center gap-1 ${op.bgColor} ${op.color} ${op.borderColor}`}>
                              <span className="text-[11px] leading-none shrink-0">{op.icon}</span>
                              <span>{op.status}</span>
                            </div>

                          </div>

                          {/* Active route info or alerts if any */}
                          {res.alert && (
                            <div className={`p-1.5 rounded text-[10px] font-mono flex items-center gap-1 border ${
                              res.alert.severity === "Crítica" ? "bg-rose-500/15 border-rose-500/20 text-rose-400 animate-pulse" : "bg-amber-500/10 border-amber-500/15 text-amber-400"
                            }`}>
                              <AlertCircle className="w-3.5 h-3.5" />
                              <span>{res.alert.msg}</span>
                            </div>
                          )}
                          ...
                          {(op.status === "Em Rota" || op.status === "Escalado") && (
                            <div className="bg-slate-950 p-2 rounded-lg border border-slate-800 space-y-1">
                              <span className="text-[9px] font-mono text-slate-400 uppercase tracking-wider block">Atividade Operacional</span>
                              {(() => {
                                const activeDt = rotas.find(r => 
                                  r.data === selectedDate && 
                                  (r.veiculoId === res.id || r.motoristaId === res.id || (r.ajudantesIds && r.ajudantesIds.includes(res.id)))
                                );
                                if (activeDt) {
                                  return (
                                    <div className="flex items-center justify-between text-[10px] font-mono text-slate-300">
                                      <span className="font-extrabold text-sky-400">DT {activeDt.numero_dt || activeDt.numero || activeDt.id}</span>
                                      <span className="text-[9px] bg-slate-800 px-1.5 py-0.5 rounded text-slate-400 truncate max-w-[120px]" title={activeDt.destino || activeDt.rotas}>
                                        📍 {activeDt.destino || activeDt.unidadeDestino || "Rota Ativa"}
                                      </span>
                                    </div>
                                  );
                                }
                                return <span className="text-[10px] text-slate-400 italic block">Vinculado em rota ativa</span>;
                              })()}
                            </div>
                          )}

                          {/* If vehicle is maintenance */}
                          {op.status === "Manutenção" && (
                            <div className="bg-amber-950/20 border border-amber-800/30 p-2 rounded-lg text-[10px] font-mono text-amber-400 leading-normal">
                              🛠️ Veículo em manutenção programada no pátio. Indisponível para carregamento até a liberação da oficina.
                            </div>
                          )}
                        ...
                        <div className="bg-slate-950/65 px-4 py-2.5 border-t border-slate-800/70 flex items-center justify-between">
                          <span className="text-[9px] text-slate-500 font-mono uppercase tracking-wider">Disponibilidade</span>
                          
                          {isBlocked ? (
                            <span className="text-[10px] text-slate-500 font-mono italic">Restrito por status</span>
                          ) : (
                            <label className="relative inline-flex items-center cursor-pointer select-none">
                              ...
                              <span className={`ml-2 text-[10px] font-extrabold font-mono uppercase tracking-wider ${
                                isChecked ? "text-emerald-400" : "text-slate-400"
                              }`}>
                                {isChecked ? "SIM" : "NÃO"}
                              </span>
      ...
      {activeTab === "cadastro" && (
        <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden space-y-4 p-4">
          ...
          {/* OFERTA DE AJUDANTES (DISPONIBILIDADE) */}
          <div className="bg-slate-900 border border-slate-800 rounded-lg overflow-hidden mt-6 mb-1 text-left">
            <div className="p-4 bg-slate-950/40 border-b border-slate-850 flex justify-between items-center">
              <div>
                <h3 className="text-xs font-bold font-mono uppercase tracking-wider text-teal-400 flex items-center gap-1.5">
                  <User className="w-4 h-4 text-teal-500" /> Oferta de Ajudantes (Fixos & Gerais)
                </h3>
                <p className="text-[10px] text-slate-500 font-sans mt-0.5">
                  Marque os ajudantes aptos que estão disponíveis para viagens na data selecionada ({selectedDate}).
                </p>
              </div>
              <span className="text-[10px] bg-teal-500/10 text-teal-400 px-2 py-0.5 rounded-full font-mono font-bold">
                {motoristas.filter(m => (m.tipo === "Ajudante Fixo" || m.tipo === "Ajudante Geral") && (selectedUnit === "Todas" || m.unidadeId === selectedUnit)).length} Cadastrados
              </span>
            </div>

            <div className="p-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 bg-slate-950/20 max-h-72 overflow-y-auto">
              {(() => {
                const targetUnit = selectedUnit === "Todas" ? "" : selectedUnit;
                const filteredHelpers = motoristas.filter(m => 
                  (m.tipo === "Ajudante Fixo" || m.tipo === "Ajudante Geral") &&
                  (!targetUnit || m.unidadeId === targetUnit)
                );

                if (filteredHelpers.length === 0) {
                  return (
                    <div className="col-span-full py-6 text-center text-slate-500 font-sans text-xs italic">
                      Nenhum ajudante cadastrado para esta filial.
                    </div>
                  );
                }

                return filteredHelpers.map(h => {
                  const isChecked = !!helpersAvailable[h.id];
                  const isBlocked = h.statusFinal === "BLOQUEADO";
                  return (
                    <label 
                      key={h.id} 
                      className={`flex items-start gap-3 p-3 rounded-lg border transition select-none cursor-pointer ${
                        isChecked 
                          ? "bg-teal-950/10 border-teal-500/30 hover:bg-teal-950/15" 
                    ...
                    >
                      <input 
                        ...
                        className="mt-0.5 rounded border-slate-800 text-teal-600 focus:ring-0 focus:ring-offset-0 w-4 h-4"
                      />
                      <div className="space-y-1">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 text-[9px] font-mono text-slate-400">
                          <span className="px-1 bg-slate-950 rounded text-slate-500">{h.cpf}</span>
                          <span className={`px-1 rounded font-bold ${h.tipo === "Ajudante Fixo" ? "bg-teal-500/10 text-teal-400" : "bg-indigo-500/10 text-indigo-400"}`}>
                            {h.tipo === "Ajudante Fixo" ? "FIXO" : "GERAL"}
                          </span>
                        </div>
                        
                        {/* Status label */}
                        <div className="flex items-center gap-1.5 pt-0.5">
                          <span className={`w-1.5 h-1.5 rounded-full ${isBlocked ? "bg-rose-500" : "bg-emerald-500 animate-pulse"}`}></span>
                          <span className={`text-[9px] font-mono ${isBlocked ? "text-rose-400" : "text-emerald-400"}`}>
                            {isBlocked ? `BLOQUEADO: ${h.motivoBloqueio || "Vencido"}` : "APTO"}
                          </span>
                        </div>
                      </div>
                    </label>
