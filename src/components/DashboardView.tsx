...
import { Unidade, Rota } from "../types";
...
export default function DashboardView({ 
  ...
  const dashboardPendingStats = useMemo(() => {
    ...
    const totalOpen = dashboardPendingDts.length;
    const critical = dashboardPendingDts.filter(r => getDaysOpen(r.data) > 10).length;
    const above5 = dashboardPendingDts.filter(r => getDaysOpen(r.data) > 5).length;

    let oldestDt = "N/A";
    let maxDays = -1;
    dashboardPendingDts.forEach(r => {
          ...
          {activeTab === "desempenho" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-8 gap-4">
                ...
                {/* Team suggestion KPI cards */}
                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <span className="text-slate-400 text-[11px] font-mono uppercase tracking-wider text-teal-400">Equipes Auto</span>
                    <Zap className="w-4 h-4 text-teal-400" />
                  </div>
                  <div className="mt-4">
                    <span className="text-2xl font-bold text-white tracking-tight">{cards.dtsWithSuggestionsCount ?? 0}</span>
                    <span className="block text-[10px] text-slate-500 mt-1">Sugestões efetuadas</span>
                  </div>
                </div>

                <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <span className="text-slate-400 text-[11px] font-mono uppercase tracking-wider text-emerald-400">Aderência Auto</span>
                    <CheckCircle className="w-4 h-4 text-emerald-400" />
                  </div>
                  <div className="mt-4">
                    <span className="text-2xl font-bold text-white tracking-tight">{cards.adherenceRateSuggestions ?? 100}%</span>
                    <span className="block text-[10px] text-emerald-500 mt-1">{cards.acceptedSuggestionsCount ?? 0} aceitas na íntegra</span>
                  </div>
                </div>
              </div>

              {/* ALERTA OPERACIONAL: CONTROLE DE DTS PENDENTES (REQUISITO 8) */}
              <div className="bg-slate-900/40 p-5 rounded-xl border border-slate-850 space-y-4 text-left">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                  ...
                  <button 
                    onClick={() => setGlobalActiveTab("fechamento-dt")}
                    className="text-[10px] bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 text-rose-300 font-mono px-3 py-1.5 rounded-lg font-bold uppercase transition flex items-center gap-1.5 cursor-pointer"
                  >
                    Ir para Módulo de Fechamento &rarr;
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  >
                    ...
                    <p className="text-[9px] text-slate-500 block font-mono mt-2">&rarr; Clique para abrir listagem</p>
                  </div>
                  ...
                  >
                    ...
                    <p className="text-[9px] text-slate-500 block font-mono mt-2">&rarr; Urgência de encerramento</p>
                  </div>

                  {/* Card 3: DTs acima de 5 dias */}
                  <div 
                    onClick={() => setGlobalActiveTab("fechamento-dt")}
                    className="p-4 bg-slate-950 hover:bg-slate-900 border border-slate-850 hover:border-slate-750 rounded-xl transition cursor-pointer flex flex-col justify-between"
                  >
                    ...
                    <p className="text-[9px] text-slate-500 block font-mono mt-2">&rarr; Prazo regulamentar expirando</p>
                  </div>

                  {/* Card 4: DT mais antiga */}
                  <div 
                    onClick={() => setGlobalActiveTab("fechamento-dt")}
                    className="p-4 bg-slate-950 hover:bg-slate-900 border border-slate-850 hover:border-slate-750 rounded-xl transition cursor-pointer flex flex-col justify-between"
                  >
                    ...
                    <p className="text-[9px] text-slate-500 block font-mono mt-2">&rarr; Prioridade máxima</p>
                  </div>
                </div>
              </div>
